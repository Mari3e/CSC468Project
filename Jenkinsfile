        ...
        stage ('Deploy') {
            ...
            steps {
                sshagent(credentials: ['cloudlab']) {
                    ...
                    sh 'ssh -o StrictHostKeyChecking=no declan@130.127.132.241 kubectl apply -f /users/declan/hasher.yaml -n jenkins'
                    sh 'ssh -o StrictHostKeyChecking=no declan@130.127.132.241 kubectl apply -f /users/declan/hasher-service.yaml -n jenkins'                                        
                }
            }
        }
    }
